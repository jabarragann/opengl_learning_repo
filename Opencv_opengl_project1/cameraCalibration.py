import numpy as np
import cv2
import glob
import argparse
import pickle
# from calibration_store import save_coefficients
from pathlib import Path 

# termination criteria
criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)


def calibrate(dirpath, square_size, width=9, height=6):
    """ Apply camera calibration operation for images in the given directory path. """
    # prepare object points, like (0,0,0), (1,0,0), (2,0,0) ....,(8,6,0)
    objp = np.zeros((height*width, 3), np.float32)
    objp[:, :2] = np.mgrid[0:width, 0:height].T.reshape(-1, 2)

    objp = objp * square_size

    # Arrays to store object points and image points from all the images.
    objpoints = []  # 3d point in real world space
    imgpoints = []  # 2d points in image plane.


    for count, fname in enumerate(dirpath.glob("*.jpg")):
        print(count, fname.name)
        fname = str(fname)
        img = cv2.imread(fname) #opencv does not read Path from pathlib directly!
        print(img.shape)
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

        # Find the chess board corners
        ret, corners = cv2.findChessboardCorners(gray, (width, height), None)

        # If found, add object points, image points (after refining them)
        if ret:
            objpoints.append(objp)

            corners2 = cv2.cornerSubPix(gray, corners, (11, 11), (-1, -1), criteria)
            imgpoints.append(corners2)

            # Draw and display the corners
            img = cv2.drawChessboardCorners(img, (width, height), corners2, ret)

            #Display img
            cv2.imshow("img", img)
            cv2.waitKey(500)

    cv2.destroyAllWindows()
    ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(objpoints, imgpoints, gray.shape[::-1], None, None)

    print(ret)
    print(mtx)
    print(dist)

    return [ret, mtx, dist, rvecs, tvecs]



if __name__ == '__main__':

    print("Camera calibration script")
    dataPath = Path("./chessboardPictures").resolve()
    parameters = calibrate(dataPath, 0.024)

    file = open('params.pickle', 'wb')
    pickle.dump(parameters, file)